...
Desktop deb release?
composeApp -> Tasks -> compose desktop -> packageReleaseDeb
...
- [ ] Convert minibuffer prompt to only have one (regardless of # of windows), it needs to take in an arg
      EX: (web-mode) -> prompts in minibuffer "URL: " -> then run "(web-mode %s)"
